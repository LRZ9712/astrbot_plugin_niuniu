...
from card_renderer import NiuniuCardRenderer
...
class NiuniuPlugin(Star):
    ...
    def __init__(self, context: Context, config: dict = None):
        ...
        self.cards = NiuniuCardRenderer(self)
        self.shop = NiuniuShop(self)  # 实例化商城模块
        self.games = NiuniuGames(self)  # 实例化游戏模块

    async def card_result(self, event, text):
        return await self.cards.result(event, text)
...
🔹 牛牛开/关 - 管理插件"""
    ...
    def get_group_data(self, group_id):
        ...
        if group_id not in data:
            data[group_id] = {'plugin_enabled': True}  # 默认关闭插件
    ...
    def update_user_data(self, group_id, user_id, updates):
        ...
        user_id = str(user_id)
        data = self._load_niuniu_lengths()
        group_data = data.setdefault(group_id, {'plugin_enabled': True})
    ...
    # region 事件处理
    niuniu_commands = ["牛牛菜单1123456", "牛牛开", "牛牛关", "注册牛牛", "打胶", "我的牛牛", "比划比划", "牛牛排行"]
    ...
    async def on_group_message(self, event: AstrMessageEvent):
        ...
        if msg.startswith("牛牛开"):
            ...
            async for result in self._toggle_plugin(event, False):
                yield result
            return
        elif msg.startswith("牛牛菜单123456"):
        ...
        # 处理其他命令
        if msg.startswith("开冲132"):
            if is_rushing:
                yield await self.card_result(event, "❌ 你已经在开冲了，无需重复操作")
                return
            async for result in self.games.start_rush(event):
                yield result
        elif msg.startswith("停止开冲123"):
            if not is_rushing:
                yield await self.card_result(event, "❌ 你当前并未在开冲，无需停止")
                return
            async for result in self.games.stop_rush(event):
                yield result
        elif msg.startswith("飞飞机123"):
            if is_rushing:
                yield await self.card_result(event, "❌ 牛牛快冲晕了，还做不了其他事情，要不先停止开冲？")
                return
            ...
            handler_map = {
                ...
                "牛牛商城123": self.shop.show_shop,
                "牛牛购买123": self.shop.handle_buy,
                "牛牛背包123": self.shop.show_items
            }
    ...
    async def on_private_message(self, event: AstrMessageEvent):
        ...
        msg = event.message_str.strip()
        niuniu_commands = [
            "牛牛菜单123456", "牛牛开", "牛牛关", "注册牛牛", "打胶", "我的牛牛",
            "比划比划", "牛牛排行", "牛牛商城123", "牛牛购买", "牛牛背包",
            "开冲", "停止开冲", "飞飞机123"  
        ]
        
        if any(msg.startswith(cmd) for cmd in niuniu_commands):
            yield await self.card_result(event, "不许一个人偷偷玩牛牛")
        else:
            return
    async def _toggle_plugin(self, event, enable):
        ...
        if not self.is_admin(user_id):
            yield await self.card_result(event, "❌ 只有管理员才能使用此指令")
            return

        self.update_group_data(group_id, {'plugin_enabled': enable})
        text_key = 'enable' if enable else 'disable'
        yield await self.card_result(event, self.niuniu_texts['system'][text_key])
    ...
    async def _dajiao(self, event: AstrMessageEvent):
        ...
        # 重新获取最新数据以显示
        user_data = self.get_user_data(group_id, user_id)
        yield await self.card_result(event, f"{final_text}\n当前长度：{self.format_length(user_data['length'])}")
    async def _compare(self, event):
        ...
        user_data = self.get_user_data(group_id, user_id)
        if not user_data:
            yield await self.card_result(event, self.niuniu_texts['dajiao']['not_registered'].format(nickname=nickname))
            return

        # 解析目标
        target_id = self.parse_target(event)
        if not target_id:
            yield await self.card_result(event, self.niuniu_texts['compare']['no_target'].format(nickname=nickname))
            return
        
        if target_id == user_id:
            yield await self.card_result(event, self.niuniu_texts['compare']['self_compare'])
            return

        # 获取目标数据
        target_data = self.get_user_data(group_id, target_id)
        if not target_data:
            yield await self.card_result(event, self.niuniu_texts['compare']['target_not_registered'])
            return
        ...
        if compare_count >= 3:
            yield await self.card_result(event, "❌ 10分钟内只能比划三次")
            return
        ...
        if user_items.get("夺心魔蝌蚪罐头", 0) > 0:
            ...
            if effect_chance < 0.5:  # 50%的概率夺取对方全部长度
                ...
                yield await self.card_result(event, "\n".join(result_msg))
                return
    ...
    async def _show_status(self, event):
        ...
        user_data = self.get_user_data(group_id, user_id)
        if not user_data:
            yield await self.card_result(event, self.niuniu_texts['my_niuniu']['not_registered'].format(nickname=nickname))
            return
    ...
    async def _show_ranking(self, event):
        ...
        if not valid_users:
            yield await self.card_result(event, self.niuniu_texts['ranking']['no_data'])
            return
    ...
    async def _show_menu(self, event):
        """显示菜单"""
        yield await self.card_result(event, self.niuniu_texts['menu']['default'])
    # endregion
